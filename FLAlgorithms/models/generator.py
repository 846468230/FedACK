...
import collections
import numpy as np
from FLAlgorithms.utils.model_config import GENERATORCONFIGS


class Generator(nn.Module):
    def __init__(self, dataset='mnist', embedding=False, latent_layer_idx=-1,visualize_boundary=False):
        ...
        self.hidden_dim, self.latent_dim, self.n_class, self.noise_dim = GENERATORCONFIGS[dataset]
    ...
    def init_loss_fn(self):
        self.crossentropy_loss= nn.CrossEntropyLoss()#nn.NLLLoss(reduce=False) # same as above
        self.mae_loss = nn.L1Loss(reduce='mean')
    ...
    def build_network(self):
        ...
        if self.visualize_boundary:
            self.representation_layer = nn.Linear(self.fc_configs[-1], 2)
        else:
