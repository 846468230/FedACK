...
import torch
import numpy as np
from FLAlgorithms.models import TeacherNet as Net
from torch.utils.data import DataLoader
from FLAlgorithms.models import Generator
from data.field import Field
from model import EModel
# from .model_config import *
METRICS = ['glob_acc', 'per_acc', 'glob_loss', 'per_loss', 'user_train_time', 'server_agg_time']

def load_lingual_model():
    from utils import parseopt,get_device
    ...
    if os.path.exists(path):
        ...
        if len(checkpoint_num) > 0:
            opt.train_from = base_path + '/' + opt.model_path + "_gpu" + str(opt.gpu) + "_warmup" + str(
                opt.warm_up) + "_latent" + str(
                opt.latent_dim) + "_kl" + str(opt.kl_annealing_steps) + "_split" + str(
                opt.split) + "/checkpoint-step-%06d" % max(checkpoint_num)
    data_path,vocab_path= opt.train, opt.vocab,
    ...
    fields = {'src': source_field, 'tgt': target_field}
    print("Build lingual model...")
    ...
    del emodel.decoder,emodel.discriminator
...
def get_dataset_name(dataset):
    ...
    if 'botometer-feedback-2019' in dataset:
        passed_dataset='botometer-feedback-2019'
    elif 'vendor-purchased-2019' in dataset:
        passed_dataset='vendor-purchased-2019'
    elif 'twibot-20' in dataset:
        passed_dataset = 'Twibot-20'
    else:
...
def create_generative_model(dataset, algorithm='', embedding=False,visualize_boundary=False):
    ...
    if 'FedGen' in algorithm:
        ...
        if 'cnn' in algorithm:
            ...
            passed_dataset += '-cnn1'
    return Generator(passed_dataset,embedding=embedding, latent_layer_idx=-1,visualize_boundary=visualize_boundary)


def create_model(model, dataset, device,only_properties,visualize_boundary):
    # passed_dataset = get_dataset_name(dataset)
    model= Net(model,dataset,device=device,only_properties=only_properties,visualize_boundary=visualize_boundary), model
    return model
...
def get_log_path(args, algorithm, seed, gen_batch_size=32,noniid_alpha=0.3):
    alg= algorithm + "_" +  args.dataset + "_" + str(noniid_alpha)
    ...
    if 'FedGKD' in algorithm:
        alg += str(args.mode)
    return alg

if __name__ =="__main__":
   a=load_lingual_model()
   print(a)